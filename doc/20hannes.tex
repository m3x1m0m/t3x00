...
Um das Modul Pop-Up Menü zu öffnen, klickt man ganz rechts oben im GPredict-Fenster auf das \myvsymbol-Symbol. Im daraufhin erscheinenden Pop-Up Menü ist es möglich die Positionierung eines Moduls innerhalb des GPredict-Fensters einzustellen, ein Modul zu kopieren, zu löschen, zu schließen oder genauer zu konfigurieren. Außerdem sind dort weitere Funktionalitäten, welche im Folgenden genauer beschrieben werden, zugänglich.\newpar
\textbf{The Sky at a Glance}\\
Wie in Abbildung \ref{fig:theskyataglance} zu sehen, bietet dieses Fenster eine Übersicht über die Zeiträume in welchen die für das Modul ausgewählten Satelliten sichtbar sind. Der Zeitraum lässt sich hierbei in den GPredict-Einstellungen unter \myemph{Predict} unter dem Reiter \myemph{Sky at a Glance} zwischen einer und 24 Stunden einstellen.
...
\textbf{Time Controller}\\
(siehe Abbildung \ref{fig:timecontroller})
...
\textbf{Moduleinstellungen (Configure)}\\
(siehe Abbildung \ref{fig:editmodule})
...
\textbf{Antenna Control}\\
(siehe Abbildung \ref{fig:rotatorcontrol})

\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{rotatorcontrol}
	\caption{Rotoren-Steuerung \textbf{Bild aktualisieren $\rightarrow$ aktive Steuerung}}
...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.5\textwidth]{radiocontrol}
	\caption{Funkgerät-Steuerung \textbf{Bild aktualisieren $\rightarrow$ aktive Steuerung}}
