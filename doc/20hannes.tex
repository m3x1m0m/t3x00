...
\begin{figure}[h]
	\centering
	\includegraphics[width=0.6\textwidth]{rotatorsettings}
	\caption{Mögliche Konfigurationen der Rotorensteuerung}
	\label{fig:rotatorconfig} 
\end{figure}

Der Elevations-Rotor lässt sich von 0\mydegree\ bis 180\mydegree\ auslenken, dementsprechend wird die Einstellung bei \myemph{Min El} und \myemph{Max El} gewählt. Der Azimut-Rotor lässt sich um 360\mydegree\ auslenken und besitzt einen Anschlag bei 180\mydegree\ (Süden), das heißt er lässt sich von 0\mydegree\ (Norden) aus jeweils um 180\mydegree\ in Richtung Süden drehen. Deshalb scheint es zunächst naheliegend beim Feld \myemph{Az type} den Bereich von -180\mydegree\ bis 180\mydegree\ einzustellen, woraufhin automatisch bei \myemph{Min Az} -180\mydegree\ und bei \myemph{Max Az} 180\mydegree\ eingetragen wird. Jedoch wurde bei verschiedenen Tests festgestellt, dass es bei der Steuerung darauf ankommt, wie der Azimut-Verlauf des Satelliten aussieht (näheres auf Seite \pageref{rotatorflip2}). Passiert ein Satellit, wie in Abbildung \ref{fig:rotatorflip} beispielhaft zu sehen, die Linie zwischen dem Mittelpunkt des Polardiagramms und der südlichen Richtung (also den Anschlag des Azimut-Rotors), ist ein Rotorflip notwendig.

\begin{figure}[h]
	\centering
	\includegraphics[width=0.3\textwidth]{rotatorflip}
