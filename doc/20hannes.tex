...
Um die Verwendung dieser Kommandozeilenprogramme zu vereinfachen und um gleichzeitig die notwendige Konfiguration festzuhalten, wurde für die Programme \myemph{rigctl}, \myemph{rigctld}, \myemph{rotctl} und \myemph{rotctld} jeweils ein Batch-Skript erstellt (siehe Anhang \ref{chap:rigctldbat}, \ref{chap:rotctldbat}, XXX und YYY). Bei der Verwendung unter Linux können diese ohne großen Aufwand in Bash-Skripte umgeschrieben werden. 
...
Durch das \myemph{NetCOM 211}-Modul und die dazugehörige Software \myemph{NetCOM Manager} ist es nicht nötig das Funkgerät per serieller Schnittstelle direkt an den verwendeten Computer anzuschließen, sondern es kann über das Hochschul-Netzwerk eine Verbindung aufgebaut werden.

\textbf{Auf Banana Pi Software eingehen}
...
siehe Anhang \ref{chap:rigctlbat} und \ref{chap:rotctlbat}
...
Für \myemph{rotctld.exe} ist unter Windows die Angabe der Localhost-IP-Adresse notwendig, da standardmäßig ,,localhost'' als IP-Adresse konfiguriert ist.\\
...
Für \myemph{rigctld.exe} ist unter Windows die Angabe der Localhost-IP-Adresse notwendig, da standardmäßig ,,localhost'' als IP-Adresse konfiguriert ist.\\
Auf COM-Port eingehen!
